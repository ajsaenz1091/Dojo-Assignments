...
SELECT concat_ws(' ',first_name,last_name) AS Name, customer.email, address.address FROM customer
...
SELECT film.title, film.description, film.release_year, film.rating, film.special_features, category.name FROM film
 ...
 SELECT actor.actor_id,CONCAT_WS(' ',actor.first_name,actor.last_name) AS actor_name, film.title AS film,film.description, film.release_year FROM actor
...
#Your query should return customer first name, last name, email, and address.
SELECT CONCAT_WS(' ',customer.first_name,customer.last_name) AS CUSTOMER, customer.email, CONCAT_WS(', ', address.address, city.city, address.district, address.postal_code) as address FROM customer
JOIN address ON customer.address_id = address.address_id
...
SELECT film.film_id,film.title, film.description, film.release_year, film.rating, film.special_features,category.name as category, actor.actor_id, CONCAT_WS(' ',actor.first_name,actor.last_name) AS actor_name FROM film
JOIN film_actor ON film.film_id = film_actor.film_id
JOIN actor ON film_actor.actor_id = actor.actor_id
JOIN film_category ON film.film_id = film_category.film_id
JOIN category ON film_category.category_id = category.category_id
