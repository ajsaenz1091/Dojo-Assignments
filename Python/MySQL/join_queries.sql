...
SELECT countries.name, countries.life_expectancy FROM countries
...
SELECT countries.name AS country, cities.name AS citie,cities.district, cities.population FROM countries
JOIN cities ON countries.id = cities.country_id
...
SELECT countries.region, COUNT(countries.id) AS amount_of_countries FROM countries
GROUP BY countries.region ORDER BY amount_of_countries DESC;
